...
fn case(rucksacks: &str) -> u32 {
    let len = rucksacks.len();
    let first_compartment = &rucksacks[0..len/2];
    let second_compartment = &rucksacks[len/2..];

    println!("{} {}", first_compartment, second_compartment);

    for c in 65..123 {
        let ch = std::char::from_u32(c).unwrap();
        if first_compartment.contains(ch) && second_compartment.contains(ch) {
            if ch.is_lowercase() {
                return c - 97 + 1;
            } else {
                return c - 65 + 27;
