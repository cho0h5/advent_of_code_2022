...
bppqwppCddlvfbDNVgmMmtMfVVmfmVWW";
    ...
    for i in 0 .. input.len() / 3 {
        ...
        // println!("{}", input[3*i+1]);
        // println!("{}", input[3*i+2]);
        let priority = case(input[3*i], input[3*i+1], input[3*i+2]);
...
fn case(first: &str, second: &str, third: &str) -> u32 {
    for c in 65..123 {
        let ch = std::char::from_u32(c).unwrap();
        if first.contains(ch) && second.contains(ch) && third.contains(ch) {
            if ch.is_lowercase() {
                return c - 97 + 1;
            } else {
                return c - 65 + 27;
